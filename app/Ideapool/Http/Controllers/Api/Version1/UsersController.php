...
use App\Ideapool\Viender\Transformers\Version1\UserTransformer;
use App\Ideapool\Http\Controllers\Api\Version1\Handlers\BasicHandler;
use App\Ideapool\Viender\Transformers\Version1\Serializer\ArraySerializer;
...
class UsersController extends ApiController
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->is_social_account) {
            ...
            if(\App\Ideapool\SocialAccount::where([
                ['provider', $request->provider],
                ['social_id', $request->social_id]
            ])->exists()) {
                $socialAccount = \App\Ideapool\SocialAccount::where([
                    ['provider', $request->provider],
                    ['social_id', $request->social_id]
                ])->first();
            ...
            // Create new social account and user if social account is not exist.
                ...
                $socialAccount = $user->socialAccounts()->save(new \App\Ideapool\SocialAccount($request->all()));
            }
